...
export const ProductForm = ({ title, subTitle, product, onSubmit, isPending }: Props) => {
  ...
  const addTag = () => {
    const newTag = labelInputRef.current!.value; // el ! le indica a typescript que siempre vamos a tener un valor
    if( newTag === '' ) return;

    const newTagSet = new Set(getValues('tags'));
    newTagSet.add(newTag);

    // Lo agrego al formulario en la parte de tags
    setValue('tags', Array.from(newTagSet))

  };

  const removeTag = (tagToRemove: string) => {
    const tagSet = new Set( getValues('tags') ) // similar al array pero No acepta valores duplicados el set
    tagSet.delete(tagToRemove);
    setValue('tags', Array.from(tagSet))
  };

  const addSize = (size: Size) => {
    const sizeSet = new Set( getValues('sizes') ) // similar al array pero No acepta valores duplicados el set
    sizeSet.add(size);
